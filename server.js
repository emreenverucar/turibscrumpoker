...
const LOGO = fs.readFileSync(path.join(__dirname, 'turib-logo.png'));
...
const isRecent = u => Date.now() - u.lastSeen < 10000;
...
function addRoom(name) {
  ...
  const room = {
    ...
    mappings: { ...baseMapping }, round: 1, revealed: false, history: [], lastActive: Date.now(), streams: new Map()
  };
  rooms.set(code, room);
  return { room, host };
}
...
function hasStream(room, token) { return !!room.streams?.get(token)?.size; }
function isConnected(room, user) { return hasStream(room, user.token) || isRecent(user); }
function writeEvent(res, event, obj) { res.write(`event: ${event}\ndata: ${JSON.stringify(obj)}\n\n`); }
function addStream(room, user, res) {
  let set = room.streams.get(user.token);
  if (!set) room.streams.set(user.token, set = new Set());
  set.add(res); user.lastSeen = Date.now(); room.lastActive = Date.now();
}
function removeStream(room, user, res) {
  const set = room.streams.get(user.token);
  if (set) { set.delete(res); if (!set.size) room.streams.delete(user.token); }
  user.lastSeen = Date.now();
}
function broadcast(room) {
  if (!room || !rooms.has(room.code)) return;
  for (const [token, set] of room.streams) {
    const user = room.members.get(token);
    if (!user) continue;
    const payload = responseFor(room, user);
    for (const res of [...set]) {
      try { writeEvent(res, 'state', payload); } catch { set.delete(res); }
    }
    if (!set.size) room.streams.delete(token);
  }
}
function closeRoomStreams(room, message='Oda kapatıldı.') {
  for (const set of room.streams.values()) for (const res of [...set]) {
    try { writeEvent(res, 'room-closed', { error: message }); res.end(); } catch {}
  }
  room.streams.clear();
}
function publicState(room, user) {
  ...
  for (const m of room.members.values()) {
    if (!isConnected(room, m)) continue;
    members[m.id] = {
...
const server = http.createServer(async (req, res) => {
  try {
    ...
    if (req.method === 'GET' && url.pathname === '/turib-logo.png') {
    ...
    if (req.method === 'GET' && url.pathname === '/api/events') {
      ...
      res.write('retry: 3000\n\n'); addStream(room, user, res);
      writeEvent(res, 'state', responseFor(room, user)); broadcast(room);
      req.on('close', () => { if (!rooms.has(room.code)) return; removeStream(room, user, res); setTimeout(() => broadcast(room), 50); });
      return;
    }
    ...
    if (url.pathname === '/api/join') {
      ...
      if (!host || !isConnected(room, host)) error(409, 'Moderatör şu anda bağlı değil. Odayı açmasını iste.');
      if ([...room.members.values()].filter(m => isConnected(room, m)).length >= 50) error(409, 'Oda dolu (en fazla 50 kişi).');
      ...
      send(res, 201, responseFor(room, user)); broadcast(room); return;
    }
    const {room,user} = authenticate(data.code, data.token);
    const hostLeaving = data.action === 'leave' && user.isHost;
    doAction(room, user, data);
    if (data.action === 'leave') {
      send(res, 200, {ok:true});
      if (hostLeaving) closeRoomStreams(room, 'Oda moderatör tarafından kapatıldı.'); else broadcast(room);
      return;
    }
    send(res, 200, responseFor(room, user)); broadcast(room);
...
const heartbeatTimer = setInterval(() => {
  for (const room of rooms.values()) for (const set of room.streams.values()) for (const res of [...set]) {
    try { res.write(': heartbeat\n\n'); } catch { set.delete(res); }
  }
}, 20 * 1000);
heartbeatTimer.unref();
const cleanTimer = setInterval(() => {
  for (const [code,room] of rooms) if (Date.now() - room.lastActive > 8 * 60 * 60 * 1000) { closeRoomStreams(room, 'Oda zaman aşımına uğradı.'); rooms.delete(code); }
}, 15 * 60 * 1000);
cleanTimer.unref();
